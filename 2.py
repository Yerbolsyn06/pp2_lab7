...
def name(n):
    font=pygame.font.SysFont("ALGERIAN",40)#шрифт
    song=font.render(mylist[n],True,(128,0,0))#pered.text
    screen.blit(song,[240,650])
    pygame.display.update()#obnovka 
...
print(mylist)             
pygame.init() 
pygame.mixer.init() #muzon.obztlno
...
SONG_END = pygame.USEREVENT + 1 #plsodnsbt
pygame.mixer.music.set_endevent(SONG_END) #svrhnvmst
...
while not done: 
    for event in pygame.event.get(): 
        ...
        if event.type == SONG_END:#psnzknch
            cnt +=1 
            if cnt == 2: 
                cnt = 0 
            else: 
